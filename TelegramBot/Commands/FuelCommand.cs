...
{
    public class FuelCommand : AbstractCommand, ICommandWithKeyboard, IExplanationString
    {
        public ReplyKeyboardMarkup Keyboard => (this as ICommandWithKeyboard).GetKeyboard(AvailableCommands, PreviousCommand);

        public override string Name => CommandName(CommandNameId.C_FUEL);
        ...
        public override ChatStates CurrentState => ChatStates.S_SET_FUEL;
        ...
        protected async override Task<Message> Action(Database.Objects.User user, TelegramBotClient client)
        {
            ...
            {
                await this.SendCollection<Fuel>(user.ChatId, db.Fuels, client);
            }
